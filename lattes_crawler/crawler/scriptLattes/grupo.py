...
from charts.graficoDeProporcoes import GraficoDeProporcoes
...
class Grupo:
	...
	def __init__(self, id_lattes, path, eraser_members = False):
		#from lattes import criarDiretorio
		from membro import Membro

#		self.arquivoConfiguracao = arquivo
		self.carregarParametrosPadrao()
		self.atualizarParametro('global-arquivo_de_entrada', id_lattes)
		self.atualizarParametro('global-diretorio_de_saida', path)
		...
		if eraser_members:
			self.listaDeMembros = []
		self.listaDeMembros.append(Membro(0, self.obterParametro('global-arquivo_de_entrada'), '', '', '', self.itemsDesdeOAno, self.itemsAteOAno, ''))
	
	def gerarXMLdeGrupo(self, save = True):
		if self.obterParametro('global-salvar_informacoes_em_formato_xml'):
			self.geradorDeXml = GeradorDeXML(self)
			xml = self.geradorDeXml.gerarXmlParaGrupo(save = save)
	...
	def carregarParametrosPadrao(self):
		...
		self.listaDeParametros.append(['global-itens_desde_o_ano', '0']) 
		self.listaDeParametros.append(['global-itens_ate_o_ano', '2014'])      # hoje
		...
		self.listaDeParametros.append(['global-salvar_informacoes_em_formato_xml', 'sim'])

		self.listaDeParametros.append(['global-identificar_publicacoes_com_qualis', 'nao'])
		self.listaDeParametros.append(['global-arquivo_qualis_de_periodicos', 'nao'])
		self.listaDeParametros.append(['global-arquivo_qualis_de_congressos', 'nao'])
		...
		self.listaDeParametros.append(['grafo-mostrar_grafo_de_colaboracoes', 'nao'])
		self.listaDeParametros.append(['grafo-mostrar_todos_os_nos_do_grafo', 'nao'])
		self.listaDeParametros.append(['grafo-considerar_rotulos_dos_membros_do_grupo', 'nao'])
		self.listaDeParametros.append(['grafo-mostrar_aresta_proporcional_ao_numero_de_colaboracoes', 'nao'])
		
		self.listaDeParametros.append(['grafo-incluir_artigo_em_periodico', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_livro_publicado', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_capitulo_de_livro_publicado', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_texto_em_jornal_de_noticia', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_trabalho_completo_em_congresso', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_resumo_expandido_em_congresso', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_resumo_em_congresso', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_artigo_aceito_para_publicacao', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_apresentacao_de_trabalho', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_outro_tipo_de_producao_bibliografica', 'nao'])

		self.listaDeParametros.append(['grafo-incluir_software_com_patente', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_software_sem_patente', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_produto_tecnologico', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_processo_ou_tecnica', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_trabalho_tecnico', 'nao'])
		self.listaDeParametros.append(['grafo-incluir_outro_tipo_de_producao_tecnica', 'nao'])

		self.listaDeParametros.append(['grafo-incluir_producao_artistica', 'nao'])
		...
		self.listaDeParametros.append(['mapa-incluir_alunos_de_mestrado', 'sim'])
